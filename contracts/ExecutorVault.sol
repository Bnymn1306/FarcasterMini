...
/**
 * @title ExecutorVault
 * @notice Vault for automated limit order execution with security controls
 * @dev Users deposit tokens → Backend executes swaps when conditions met → Users withdraw results
 */
contract ExecutorVault is Ownable, ReentrancyGuard {
    ...
    // Approved tokens (limits to audited tokens)
    ...
    /**
     * @notice Execute swap on behalf of user (only callable by backend executor)
    ...
    function executeSwap(
        ...
        require(approvedTokens[fromToken], "From token not approved");
        require(approvedTokens[toToken], "To token not approved");
        ...
        // Verify fromToken balance decreased correctly (detect fee-on-transfer)
        ...
        require(actualFromSpent >= fromAmount, "Unexpected fromToken balance");
        ...
        emit SwapExecuted(user, fromToken, toToken, fromAmount, toAmount, orderId);
    ...
    /**
     * @notice Add or remove approved token (only owner)
